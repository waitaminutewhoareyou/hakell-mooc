...
takeFinal n xs = drop k xs
                 where k = ( if n > length xs then 0 else length xs - n)
...
isPalindrome str
 | (length str <= 1) = True
 | (head str == head(reverse str)) = isPalindrome (init (tail str))
 | otherwise = False
...
addEithers (Right int) (Left str) = Left str
addEithers a b = a
