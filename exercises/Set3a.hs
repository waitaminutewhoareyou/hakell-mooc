...
firstHalf str = take (len `div` 2) str
                where len =  length str + (length str `mod` 2 )
...
capitalize str =  unwords ( map f (words str) )

f :: String -> String
f str = [toUpper (head str)] ++ tail str
------------------------------------------------------------------------------
...
whileRight check x = case check x of Left result -> result
                                     Right intermediate -> whileRight check intermediate
...
joinToLength n ls = filter (\x -> length x ==n) [ i ++ j | i <- ls, j <- ls]
...
sumRightsHelper (Right int) = int
sumRightsHelper (Left asshole) = 0

sumRights :: [Either a Int] -> Int
sumRights ls = sum $ map sumRightsHelper ls
...
-- foldr takes the second argument and the last item of the list and applies the
-- function, then it takes the penultimate item from the end and the result, and
-- so on.
...
interpreterHelper commands x y res
    | null commands = map show res
    |  head commands == "printY" = interpreterHelper (tail commands) x y (res ++ [y])
    |  head commands == "printX" = interpreterHelper (tail commands) x y (res ++ [x])
    |  head commands == "up" = interpreterHelper (tail commands) x (y+1) res
    |  head commands == "down" = interpreterHelper (tail commands) x (y-1) res
    |  head commands == "right" = interpreterHelper (tail commands) (x+1) y res
    |  head commands == "left" = interpreterHelper (tail commands) (x-1) y res
